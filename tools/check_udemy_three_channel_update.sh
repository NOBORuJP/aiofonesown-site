...
set -euo pipefail
FAIL=0
for f in index.html en/index.html ja/index.html updates/index.html news/index.html css/style.css; do [ -f "$f" ] || { echo "FAIL missing $f"; FAIL=1; }; done
...
if grep -q "Book · Course · Template Pack" index.html && grep -q "Chronological release record" updates/index.html; then echo "OK wording"; else echo "FAIL wording"; FAIL=1; fi
