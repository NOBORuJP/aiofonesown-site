...
export const readingPages = [
  ...
  {id:'history-people',no:'人',name:'和算家たち',sub:'ひとりの天才を越えて',title:'問いを受け取り、先へ渡す人。',intro:'本を書く人、算法を広げる人、記録を残す人。何をした人なのかから、和算家たちに出会ってみましょう。',body:()=>`
    <div class="reading-people">
      ...
      <article><p class="reading-label">式を扱う力を広げる</p><h2>関孝和 <span>せき たかかず</span></h2><p class="reading-dates">生年未詳–1708 <span>「こうわ」とも読む</span></p><p>数字と文字を組み合わせる傍書法を発展させ、複数の未知数を含む問題の消去を進めました。円周率や累乗和などにも仕事を残しています。生年には異説があるため、確定した年として掲げません。</p>${cite('seki')}${tryTool('tengen','未知数と係数の考え方に触れる')}${tryTool('elimination','二つの未知数から、一つを消す')}${tryTool('bernoulli','和を作る係数を調べる')}${tryTool('replacement','関の換式を使う')}</article>
      <article><p class="reading-label">数から計算法を探る</p><h2>建部賢弘 <span>たけべ かたひろ</span></h2><p class="reading-dates">1664–1739</p><p>関の門人。関の著作を解説・継承し、自身も円周率や円弧を研究しました。『綴術算経』では、何を調べ、どのように調べるかという数学研究の方法にも目を向けています。</p>${cite('seki','philosophy')}${tryTool('enri','近似値の変わり方を比べる')}${tryTool('arc','円弧の級数を計算する')}</article>
      ...
      <article><p class="reading-label">円理を次の世代へ</p><h2>松永良弼 <span>まつなが よしすけ</span></h2><p class="reading-dates">生年未詳–1744（資料により1690年頃、1694?など）</p><p>関の没後の世代に円理を発展させた和算家。生年は資料で一致しておらず、本サイトでは一つに確定しません。</p><div class="reading-citations"><a href="https://kotobank.jp/word/松永良弼-136693" target="_blank" rel="noopener noreferrer">コトバンク｜松永良弼 ↗</a><a href="https://www.combinatorics.org/ojs/index.php/eljc/article/view/v29i2p2" target="_blank" rel="noopener noreferrer">Dou・Hwang・Li（2022） ↗</a></div></article>
      ...
      <article><p class="reading-label">後期円理を押し広げる</p><h2>和田寧 <span>わだ やすし（「ねい」とも）</span></h2><p class="reading-dates">1787–1840</p><p>円理の計算法を発展させ、後続の和算家に大きな影響を与えました。「円理」は関の時代だけで完成した一つの公式ではなく、世代を重ねて育った研究分野です。</p>${cite('later','chronology')}</article>
  ...
  {id:'history-stories',no:'話',name:'和算の蘊蓄',sub:'知るほど面白い小話',title:'計算の向こうに、人がいる。',intro:'答えを載せない本。数学を運ぶ旅人。元の額がなくても生き残った問題。和算の面白さは、式の外にもあります。',body:()=>`
    <div class="reading-stories">
      ...
      <article><p class="reading-label">09 / 源氏香</p><h2>五つの香りから、52通り。</h2><p>源氏香の図は、五つの香の同じ・違うを52通りに整理します。現代数学では、五つの異なる対象を空でない組に分ける総数はベル数 B₅=52 です。江戸の人が「ベル数」という名称を使ったわけではありません。後世の数学史研究が、和算の数え上げと現代の組合せ数との対応を調べています。</p>${tryTool('bell','ベル数52を計算する')}</article>
