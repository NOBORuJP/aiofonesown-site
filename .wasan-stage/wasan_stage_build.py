...
def main():
    ...
    # Distribution-package structure checks. These catch a package that looks
    # fine at the site root but is incomplete after a user downloads it.
    site_expect={
        'AI-NOBORu-wasan-site.zip': 100,
        'AI-NOBORu-wasan-site-en.zip': 100,
    }
    for zname,count in site_expect.items():
        zp=DIST/zname
        with zipfile.ZipFile(zp) as z:
            names=[n for n in z.namelist() if not n.endswith('/')]
            if len(names)!=count:
                raise RuntimeError(f'{zname}: expected {count} files, got {len(names)}')
            # Site ZIPs must not recursively contain any distribution ZIP.
            if any('/downloads/AI-NOBORu-wasan-site.zip' in n or '/downloads/AI-NOBORu-wasan-site-en.zip' in n or '/downloads/AI-NOBORu-aiofonesown-kit.zip' in n for n in names):
                raise RuntimeError(zname+': recursive distribution zip remains')
            if not any(n.endswith('/SHA256SUMS.json') for n in names):
                raise RuntimeError(zname+': manifest missing')

    kit=DIST/'AI-NOBORu-aiofonesown-kit.zip'
    with zipfile.ZipFile(kit) as z:
        names=[n for n in z.namelist() if not n.endswith('/')]
        if len(names)!=109:
            raise RuntimeError(f'installation kit: expected 109 files, got {len(names)}')
        for required in ['wasan/downloads/AI-NOBORu-wasan-site.zip','wasan/downloads/AI-NOBORu-wasan-site-en.zip']:
            if required not in names:
                raise RuntimeError('installation kit missing '+required)
        if 'wasan/downloads/AI-NOBORu-aiofonesown-kit.zip' in names:
            raise RuntimeError('installation kit recursively contains itself')
        if 'SHA256SUMS.json' not in names or 'wasan/SHA256SUMS.json' not in names:
            raise RuntimeError('installation kit manifest missing')

    print('WASAN_STAGE_BUILD_PASS files=103 manifest='+str(len(rootman))+' site_packages=100/100 kit=109')
